...
//****************************************************************************
//JL: Para debug de los equipos escuchando en por 485. 
...
void RadioTask(void)
{
    ...
    if( MiApp_MessageAvailable())   
    {
        if((rxMessage.Payload[1] <= PCDevices)&&((rxMessage.Payload[3] == OutputStat))&&((rxMessage.Payload[4] <= 1))&&((rxMessage.Payload[5] <= 200)))
        {
            ...
            ConsolePut(EndCmd);
            //*****************************************************************
            //JL: Replica para el bus 485 para monitoreo de los dispositivos conectados
            DRV_485 = 1;
            
            Rs485Put(rxMessage.Payload[1]);
            while(!TXSTA1bits.TRMT);
            DRV_485 = 0;
            //*****************************************************************
